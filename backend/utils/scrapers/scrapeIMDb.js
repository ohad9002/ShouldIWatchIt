...
async function scrapeIMDb(page, movieTitle) {
  ...
  return await retry(async () => {
    ...
    // 1️⃣ Try to grab visible title links immediately
    ...
    if (!candidates.length) {
      console.warn('⚠️ [IMDb] No links → trying JSON-LD fallback');
      const ldHandle = await page.$('script[type="application/ld+json"]');
      if (ldHandle) {
        const raw = await ldHandle.evaluate(n => n.textContent);
        try {
    ...
    factor:  2,
    jitter:  true,
    // only retry on navigation/timeouts, not on empty selectors
    shouldRetry: err => err.name === 'TimeoutError' && /navigation|networkidle/.test(err.message)
  });
}
