...
async function scrapeOscars(page, movieTitle) {
  ...
  const films = await page.$$eval(
    ...
    (nodes) => nodes.map(f => {
      ...
      return { title: t, normalizedTitle: t.toLowerCase().replace(/[^a-z0-9\s]/g, '').trim(), url: a?.href || '' };
    })
  );
  ...
  films.forEach(f => f.similarity = calculateSimilarity(f.normalizedTitle, norm));
  ...
  // Extract nominations only for that film
